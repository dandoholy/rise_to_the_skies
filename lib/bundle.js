...
class Avatar {
  constructor(props) {
    ...
    this.image.src = "./assets/images/fiery_tower_character2.png"
    this.pos = props.pos;
    this.vel = [0, 0];
    this.jumping = false;
    this.onLand = false;
...
class Game {
  ...
  removePlatform() {
    if (this.time < 40) {
      if (Math.floor(this.time%4) === 1) {
        this.platforms.shift(); }
    }
    else if (this.time < 80) {
      if (Math.floor(this.time%3) === 1) { this.platforms.shift(); }
    }
    else if (this.time < 120) {
      if (Math.floor(this.time%2) === 1) { this.platforms.shift(); }
    }
    else {
      if (Math.floor(this.time%1) === 1) { this.platforms.shift(); }
    }
  }
  ...
  updateScore() {
    ...
    this.scoreBoard.innerHTML = `Score: ${this.score}` + "<br />" + `Time: ${this.time.toFixed(2)}`;
  }
}
